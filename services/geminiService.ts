...
// In-memory cache for writing rules (5 min TTL)
let cachedWritingRules: string | null = null;
let writingRulesCacheExpiry = 0;

const getWritingRules = async (): Promise<string> => {
  const now = Date.now();
...
const processPressRelease = async (
  ...
  const writingRules = await getWritingRules();
