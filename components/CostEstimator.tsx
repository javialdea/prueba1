...
import { X, Euro, TrendingUp, Calculator, Info, Zap, Mic, FileText, MessageSquare, Newspaper, MapPin, Mail, Activity, Trash2 } from 'lucide-react';
import {
    calculateAudioCost,
...
export const CostEstimator: React.FC<CostEstimatorProps> = ({ isOpen, onClose }) => {
    ...
    const [avgAudioMinutes, setAvgAudioMinutes] = useState(12);
    const [avgDocumentSizeKB, setAvgDocumentSizeKB] = useState(50);

    // Real token usage from tokenStore — updates live as operations complete
    const [realEntries, setRealEntries] = useState<TokenEntry[]>(() => getTokenEntries());
    const realTotals = getSessionTotals();

    useEffect(() => {
        const unsubscribe = subscribeToTokenEntries(() => {
            setRealEntries([...getTokenEntries()]);
        });
        return unsubscribe;
    }, []);
            ...
            <div className="flex min-h-full items-center justify-center p-4">
                <div className="bg-white w-full max-w-5xl rounded-[2.5rem] shadow-2xl overflow-hidden border border-servimedia-border animate-in zoom-in-95 duration-300 my-8">
                    ...
                    <div className="p-10 space-y-10">
                        ...
                        <div className="rounded-3xl overflow-hidden border-2 border-blue-200 shadow-sm">
                            <div className="px-6 py-4 border-b border-blue-100 bg-gradient-to-r from-blue-50 to-indigo-50 flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    <div className="w-8 h-8 bg-blue-600 rounded-xl flex items-center justify-center text-white">
                                        <Activity className="w-4 h-4" />
                                    </div>
                                    <div>
                                        <p className="text-sm font-black text-blue-800 uppercase tracking-wider">Uso Real — Sesión Actual</p>
                                        <p className="text-[9px] text-blue-500/70 font-bold uppercase tracking-widest mt-0.5">
                                            {realEntries.length} {realEntries.length === 1 ? 'llamada' : 'llamadas'} · tokens reales de usageMetadata
                                        </p>
                                    </div>
                                </div>
                                {realEntries.length > 0 && (
                                    <button
                                        onClick={() => { clearTokenEntries(); setRealEntries([]); }}
                                        className="flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-black uppercase tracking-wider text-red-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all"
                                    >
                            ...
                            {realEntries.length === 0 ? (
                                <div className="px-6 py-10 text-center">
                                    ...
                                    <p className="text-sm text-servimedia-gray/30 font-bold">Aún no se ha realizado ninguna operación en esta sesión</p>
                                    <p className="text-[10px] text-servimedia-gray/20 mt-1">Los datos aparecerán aquí en tiempo real al usar la app</p>
                                </div>
                            ) : (
                                <>
                                    <div className="overflow-x-auto">
                                        <table className="w-full text-left text-xs">
                                            <thead className="bg-blue-50/50">
                                                <tr>
                                                    <th className="px-5 py-3 font-black uppercase tracking-widest text-[9px] text-blue-400">Operación</th>
                                                    <th className="px-5 py-3 font-black uppercase tracking-widest text-[9px] text-blue-400">Hora</th>
                                                    <th className="px-5 py-3 font-black uppercase tracking-widest text-[9px] text-blue-400 text-right">Tokens entrada</th>
                                                    <th className="px-5 py-3 font-black uppercase tracking-widest text-[9px] text-blue-400 text-right">Tokens salida</th>
                                                    <th className="px-5 py-3 font-black uppercase tracking-widest text-[9px] text-blue-400 text-right">Total tokens</th>
                                                    <th className="px-5 py-3 font-black uppercase tracking-widest text-[9px] text-blue-400 text-right">Coste real</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-blue-50">
                                                {[...realEntries].reverse().map(entry => (
                                                    <tr key={entry.id} className="hover:bg-blue-50/30 transition-colors">
                                                        <td className="px-5 py-3 font-bold text-servimedia-gray">{entry.label}</td>
                                                        <td className="px-5 py-3 text-servimedia-gray/40 tabular-nums">
                                                            {entry.timestamp.toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
                                                        </td>
                                                        <td className="px-5 py-3 text-right tabular-nums text-servimedia-gray/60">{entry.inputTokens.toLocaleString('es-ES')}</td>
                                                        <td className="px-5 py-3 text-right tabular-nums text-servimedia-gray/60">{entry.outputTokens.toLocaleString('es-ES')}</td>
                                                        <td className="px-5 py-3 text-right tabular-nums font-bold text-servimedia-gray">{entry.totalTokens.toLocaleString('es-ES')}</td>
                                                        <td className="px-5 py-3 text-right tabular-nums font-black text-blue-700">{formatCurrency(entry.costEUR)}</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
                                    ...
                                    <div className="px-6 py-5 bg-gradient-to-r from-blue-50 to-indigo-50 border-t border-blue-100">
                                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                            <div>
                                                <span className="text-[9px] font-black uppercase tracking-wider text-blue-400 block mb-1">Llamadas</span>
                                                <span className="text-xl font-black text-blue-800">{realEntries.length}</span>
                                            </div>
                                            <div>
                                                <span className="text-[9px] font-black uppercase tracking-wider text-blue-400 block mb-1">Tokens entrada</span>
                                                <span className="text-xl font-black text-blue-800">{realTotals.totalInputTokens.toLocaleString('es-ES')}</span>
                                            </div>
                                            <div>
                                                <span className="text-[9px] font-black uppercase tracking-wider text-blue-400 block mb-1">Tokens salida</span>
                                                <span className="text-xl font-black text-blue-800">{realTotals.totalOutputTokens.toLocaleString('es-ES')}</span>
                                            </div>
                                            <div>
                                                <span className="text-[9px] font-black uppercase tracking-wider text-blue-400 block mb-1">Coste sesión</span>
                                                <span className="text-2xl font-black text-blue-700">{formatCurrency(realTotals.totalCostEUR)}</span>
                                            </div>
                                        </div>
                                    </div>
                                </>
